(function () {
  ...
  const LEARN = window.LEARNING_DATA;
  ...
  const defaultState = { schema: 2, completed: [], bookmarks: [], activity: [], cpd: [], quiz: {}, readiness: {}, masteryEvidence: {}, labProgress: {}, theme: "light", lastWeek: 1, learner: { startingPoint: "", objective: "", selectedPath: "", diagnostic: {}, diagnosticVersion: "", assessment: {}, capstone: { milestones: {}, evidence: {} } }, tutor: { depth: "learner", messages: [], lastTopic: "", lastDocs: [], lastIntent: "", pendingCalculation: null } };
  let state = loadState();
  ...
  function loadState() {
    try {
      ...
      if (!parsed || ![1,2].includes(parsed.schema) || !Array.isArray(parsed.completed) || !Array.isArray(parsed.bookmarks)) return structuredClone(defaultState);
      ...
      migrated.learner = { ...structuredClone(defaultState.learner), ...(parsed.learner || {}) };
      migrated.learner.diagnostic = migrated.learner.diagnostic && typeof migrated.learner.diagnostic === "object" ? migrated.learner.diagnostic : {};
      migrated.learner.assessment = migrated.learner.assessment && typeof migrated.learner.assessment === "object" ? migrated.learner.assessment : {};
      migrated.learner.capstone = { ...structuredClone(defaultState.learner.capstone), ...(migrated.learner.capstone || {}) };
      migrated.learner.capstone.milestones = migrated.learner.capstone.milestones && typeof migrated.learner.capstone.milestones === "object" ? migrated.learner.capstone.milestones : {};
      migrated.learner.capstone.evidence = migrated.learner.capstone.evidence && typeof migrated.learner.capstone.evidence === "object" ? migrated.learner.capstone.evidence : {};
  ...
  let pendingStateImport = null;
  function pathFor(id) { return LEARN.paths.find(path => path.id === id); }
  function diagnosticComplete() { return LEARN.diagnostic.every(q => Number.isInteger(state.learner.diagnostic[q.id])); }
  function diagnosticScore() {
    const scores = Object.fromEntries(LEARN.paths.map(path => [path.id, 0]));
    LEARN.diagnostic.forEach(question => { if (state.learner.diagnostic[question.id] === question.answer) question.tags.forEach(tag => { scores[tag] += 1; }); });
    return scores;
  }
  function recommendedPath() {
    const preferred = LEARN.objectives.find(item => item.id === state.learner.objective)?.path;
    if (!diagnosticComplete()) return preferred || (state.learner.startingPoint === "new" ? "A" : "H");
    const score = diagnosticScore();
    const best = Object.keys(score).sort((a, b) => score[b] - score[a] || (a === preferred ? -1 : b === preferred ? 1 : a.localeCompare(b)))[0];
    if (state.learner.startingPoint === "new" && score.A < 4) return "A";
    return preferred || best || "A";
  }
  function selectedPath() { return pathFor(state.learner.selectedPath || recommendedPath()) || LEARN.paths[0]; }
  function pathStatus(path) {
    const completed = path.chapters.filter(isComplete).length;
    return { completed, total: path.chapters.length, percent: Math.round((completed / path.chapters.length) * 100), next: path.chapters.find(n => !isComplete(n)) || path.chapters[path.chapters.length - 1] };
  }
  function renderStart() {
    const profile = LEARN.profiles.find(item => item.id === state.learner.startingPoint);
    const objective = LEARN.objectives.find(item => item.id === state.learner.objective);
    const recommended = pathFor(recommendedPath());
    const score = diagnosticComplete() ? diagnosticScore() : null;
    $("#startView").innerHTML = `<div class="page-heading"><div><p class="eyebrow">Start Here</p><h1>Build your learning route before you build a model.</h1><p>GridReady UK is a self-learning and training environment for electrical engineers. It teaches the central concepts, calculations, study workflows and reporting practice; it does not certify competence or replace current approved project documents.</p></div></div>
      <section class="onboarding-grid"><article class="onboarding-card"><h2>How to use this platform</h2><ol><li>Choose a starting position and objective.</li><li>Complete the six-question diagnostic without looking up answers.</li><li>Follow the advisory route, while keeping all chapters open.</li><li>Retain calculations, model evidence and conclusions in your own portfolio.</li></ol><p><strong>Prerequisites:</strong> basic algebra; the Foundation Zero material covers the electrical and mathematical starting point. PowerFactory is optional for reading, but a licensed installation is needed for practical labs. Your selections and progress stay in this browser unless you export them.</p></article><article class="onboarding-card"><h2>Training boundary</h2><p>Use the continuing network and all values as training assumptions. For live work, use the current code/standard, connection agreement, DNO/TO/NESO instruction, manufacturer data, approved model and competent review.</p><button class="secondary-button" data-go="sources">Open controlled source library</button></article></section>
      <section class="setup-card"><h2>1. Tell the platform where you are starting</h2><form id="routeSetupForm"><fieldset><legend>Starting position</legend>${LEARN.profiles.map(item => `<label class="choice-card"><input type="radio" name="startingPoint" value="${esc(item.id)}" ${state.learner.startingPoint === item.id ? "checked" : ""}><span><strong>${esc(item.label)}</strong><small>${esc(item.note)}</small></span></label>`).join("")}</fieldset><fieldset><legend>Main objective</legend>${LEARN.objectives.map(item => `<label class="choice-card"><input type="radio" name="objective" value="${esc(item.id)}" ${state.learner.objective === item.id ? "checked" : ""}><span>${esc(item.label)}</span></label>`).join("")}</fieldset><button class="primary-button" type="submit">Save route choices</button></form>${profile || objective ? `<p class="route-note"><strong>Current selection:</strong> ${esc(profile?.label || "Choose a starting position")} · ${esc(objective?.label || "Choose an objective")}</p>` : ""}</section>
      <section class="diagnostic-card"><div class="section-heading"><div><p class="eyebrow">Diagnostic assessment</p><h2>2. Check your starting point</h2><p>This is not a pass/fail exam. It checks concepts, method selection and engineering judgement, then recommends a review route.</p></div>${diagnosticComplete() ? `<span class="tag">Completed</span>` : `<span class="tag">6 questions</span>`}</div><form id="diagnosticForm">${LEARN.diagnostic.map((q, i) => `<fieldset class="diagnostic-question"><legend>${i + 1}. ${esc(q.prompt)}</legend>${q.options.map((option, choice) => `<label><input type="radio" name="diagnostic-${q.id}" value="${choice}" ${state.learner.diagnostic[q.id] === choice ? "checked" : ""}> ${esc(option)}</label>`).join("")}</fieldset>`).join("")}<button class="primary-button" type="submit">Check diagnostic and recommend a route</button></form>${score ? `<div class="diagnostic-result"><h3>Recommendation: Path ${esc(recommended.id)} — ${esc(recommended.name)}</h3><p>${esc(recommended.outcome)}</p><p><strong>Diagnostic:</strong> ${LEARN.diagnostic.filter(q => state.learner.diagnostic[q.id] === q.answer).length}/6 concept/method questions correct. Use incorrect answers as revision prompts, not as a measure of professional competence.</p><button class="secondary-button" data-select-path="${esc(recommended.id)}">Use this advisory route</button></div>` : ""}</section>
      <section class="data-controls"><div><p class="eyebrow">Your browser-local data</p><h2>Progress portability and reset</h2><p>Export before moving browsers or clearing site data. Import is previewed before it replaces this browser’s record. Do not import untrusted files.</p></div><div class="control-buttons"><button class="secondary-button" id="exportProgress" type="button">Export progress JSON</button><label class="secondary-button file-label">Preview progress import<input id="importProgress" type="file" accept="application/json,.json"></label><button class="text-button danger-button" data-reset-scope="lessons" type="button">Reset chapter reading</button><button class="text-button danger-button" data-reset-scope="assessments" type="button">Reset diagnostics/capstone</button><button class="text-button danger-button" data-reset-scope="all" type="button">Reset all local data</button></div><div id="importPreview" aria-live="polite"></div></section>`;
    bindCommon($("#startView"));
    $("#routeSetupForm").addEventListener("submit", event => { event.preventDefault(); const form = new FormData(event.currentTarget); state.learner.startingPoint = String(form.get("startingPoint") || ""); state.learner.objective = String(form.get("objective") || ""); if (!state.learner.selectedPath) state.learner.selectedPath = recommendedPath(); logActivity("Updated learning-route choices"); saveState(); renderStart(); renderDashboard(); });
    $("#diagnosticForm").addEventListener("submit", event => { event.preventDefault(); const answers = {}; LEARN.diagnostic.forEach(q => { const selected = new FormData(event.currentTarget).get(`diagnostic-${q.id}`); if (selected !== null) answers[q.id] = Number(selected); }); if (Object.keys(answers).length !== LEARN.diagnostic.length) { showToast("Answer all six diagnostic questions first."); return; } state.learner.diagnostic = answers; state.learner.diagnosticVersion = LEARN.version; state.learner.selectedPath = recommendedPath(); logActivity(`Completed diagnostic; advisory Path ${state.learner.selectedPath}`); saveState(); renderStart(); renderDashboard(); });
    $$('[data-select-path]', $("#startView")).forEach(button => button.addEventListener("click", () => { state.learner.selectedPath = button.dataset.selectPath; saveState(); renderStart(); renderPaths(); renderDashboard(); showToast(`Path ${button.dataset.selectPath} selected. All material remains open.`); }));
    $("#exportProgress").addEventListener("click", exportProgress);
    $("#importProgress").addEventListener("change", previewProgressImport);
    $$('[data-reset-scope]', $("#startView")).forEach(button => button.addEventListener("click", () => resetProgress(button.dataset.resetScope)));
  }
  function renderPaths() {
    const active = selectedPath(); const status = pathStatus(active);
    $("#pathsView").innerHTML = `<div class="page-heading"><div><p class="eyebrow">Learning paths</p><h1>Choose an advisory route, not a content gate.</h1><p>Each path reuses the technical book, labs and assessments in a different sequence. Prerequisites guide study order; they never lock a chapter.</p></div></div><section class="selected-path"><p class="eyebrow">Your selected route</p><h2>Path ${esc(active.id)} — ${esc(active.name)}</h2><p>${esc(active.outcome)}</p><div class="path-progress"><strong>${status.percent}% chapter reading on this path</strong><span>${status.completed}/${status.total} mapped chapters read</span></div><button class="primary-button" data-open-week="${status.next}">Next route chapter: ${status.next} ${icon("arrow")}</button></section><div class="path-grid">${LEARN.paths.map(path => { const info = pathStatus(path); return `<article class="path-card ${path.id === active.id ? "is-selected" : ""}"><span class="tag">Path ${esc(path.id)}</span><h2>${esc(path.name)}</h2><p>${esc(path.audience)}</p><details><summary>Route detail</summary><p><strong>Advisory prerequisite:</strong> ${esc(path.prerequisite)}</p><p><strong>Estimated effort:</strong> ${esc(path.hours)} h</p><p><strong>Chapters:</strong> ${esc(path.chapters.join(", "))}</p><p><strong>Required labs:</strong> ${esc(path.labs.join("; "))}</p><p><strong>Portfolio:</strong> ${esc(path.portfolio.join("; "))}</p><p><strong>Completion standard:</strong> ${esc(path.standard)}</p></details><div class="path-progress"><strong>${info.percent}%</strong><span>${info.completed}/${info.total} chapters read</span></div><button class="secondary-button" data-select-path="${esc(path.id)}">${path.id === active.id ? "Selected" : "Select this route"}</button></article>`; }).join("")}</div>`;
    $$('[data-select-path]', $("#pathsView")).forEach(button => button.addEventListener("click", () => { state.learner.selectedPath = button.dataset.selectPath; logActivity(`Selected Path ${state.learner.selectedPath}`); saveState(); renderPaths(); renderDashboard(); })); bindCommon($("#pathsView"));
  }
  function renderProject() {
    const project = LEARN.project;
    $("#projectView").innerHTML = `<div class="page-heading"><div><p class="eyebrow">Continuing PowerFactory training project</p><h1>${esc(project.name)}</h1><p>${esc(project.boundary)}</p></div></div><section class="project-brief"><div><h2>Project brief</h2><p>${esc(project.brief)}</p><p><strong>Use it this way:</strong> build the model independently from the controlled training data; do not treat it as a supplied proprietary PowerFactory file or a real-network design.</p></div><div class="project-sld" role="img" aria-label="Simplified Northmere Energy Park single-line diagram"><svg viewBox="0 0 760 250" aria-hidden="true"><path d="M60 120H180M250 120H360M430 120H540M610 120H710"/><rect x="180" y="82" width="70" height="76" rx="8"/><rect x="360" y="82" width="70" height="76" rx="8"/><rect x="540" y="82" width="70" height="76" rx="8"/><text x="24" y="104">132 kV</text><text x="170" y="195">T1/T2</text><text x="344" y="195">33 kV bus</text><text x="525" y="195">BESS / solar</text><text x="650" y="104">11 kV loads</text></svg><p>Simplified teaching SLD — confirm every terminal, cubicle, switch and model boundary in your own model.</p></div></section><div class="two-column"><article><h2>Controlled assets</h2><ul>${project.assets.map(item => `<li>${esc(item)}</li>`).join("")}</ul></article><article><h2>Operating scenarios</h2><ol>${project.scenarios.map(item => `<li>${esc(item)}</li>`).join("")}</ol></article></div><section class="project-registers"><h2>Project registers to maintain</h2><div class="tag-list">${project.registers.map(item => `<span>${esc(item)}</span>`).join("")}</div><h3>How each book part contributes</h3><dl>${Object.entries(project.chapterLinks).map(([range, value]) => `<dt>Chapters ${esc(range)}</dt><dd>${esc(value)}</dd>`).join("")}</dl></section><section class="clinic-section"><div class="section-heading"><div><p class="eyebrow">Study clinic</p><h2>Diagnose a plausible but questionable result</h2><p>Use a case to practise the order of investigation before relying on a software result.</p></div></div><div class="clinic-grid">${LEARN.clinic.map(caseItem => `<article><span class="tag">${esc(caseItem.area)}</span><h3>${esc(caseItem.title)}</h3><p><strong>Symptom:</strong> ${esc(caseItem.symptom)}</p><details><summary>Reveal diagnostic sequence</summary><p><strong>Check first:</strong> ${esc(caseItem.check)}</p><p><strong>Risk:</strong> ${esc(caseItem.risk)}</p><p>Write the physical explanation, an independent check, required data and a cautious report conclusion before revealing a final answer.</p></details></article>`).join("")}</div></section>`;
  }
  function templateContent(template) { const heading = `# ${template.title}\n\nTraining-only GridReady UK template. Replace all prompts with controlled project information; this does not replace an employer/DNO/TO template.\n\n`; return template.format === "csv" ? `${template.fields.join(",")}\n` : `${heading}${template.fields.map(field => `## ${field}\n\n[Enter controlled information, source/revision and reviewer where applicable.]\n`).join("\n")}`; }
  function downloadText(filename, content, type = "text/plain") { const url = URL.createObjectURL(new Blob([content], { type })); const link = document.createElement("a"); link.href = url; link.download = filename; document.body.appendChild(link); link.click(); link.remove(); setTimeout(() => URL.revokeObjectURL(url), 0); }
  function renderCapstone() {
    const capstone = LEARN.capstone; const complete = capstone.deliverables.filter((_, index) => state.learner.capstone.milestones[index]).length;
    $("#capstoneView").innerHTML = `<div class="page-heading"><div><p class="eyebrow">Final project</p><h1>Prepare a defensible training study package.</h1><p>${esc(capstone.warning)}</p></div></div><section class="capstone-summary"><h2>Six milestones; 25 controlled deliverables</h2><p>Progress records only that you have retained your own evidence. It does not assess the technical correctness of that evidence.</p><div class="path-progress"><strong>${complete}/${capstone.deliverables.length} deliverables marked as evidence retained</strong><span>Use a competent review before any live issue.</span></div><ol>${capstone.milestones.map(item => `<li>${esc(item)}</li>`).join("")}</ol></section><section class="capstone-deliverables"><h2>Deliverable tracker</h2>${capstone.deliverables.map((item, index) => `<label class="capstone-item"><input type="checkbox" data-capstone-item="${index}" ${state.learner.capstone.milestones[index] ? "checked" : ""}><span><strong>${index + 1}. ${esc(item)}</strong><small>Retain a controlled file, calculation, result extract or review note. Record limitations and open actions.</small></span></label>`).join("")}</section><section class="rubric-card"><h2>Self-review before you call the package ready for review</h2><ul>${capstone.rubric.map(item => `<li>${esc(item)}</li>`).join("")}</ul></section><section class="templates-section"><div class="section-heading"><div><p class="eyebrow">Open-format templates</p><h2>Download a training template</h2><p>These generic CSV/Markdown files are intentionally not controlled employer templates.</p></div></div><div class="template-grid">${LEARN.templates.map(template => `<article><span class="tag">.${esc(template.format)}</span><h3>${esc(template.title)}</h3><p>${esc(template.fields.slice(0,3).join(" · "))}${template.fields.length > 3 ? " …" : ""}</p><button class="secondary-button" data-template="${esc(template.id)}">Download ${esc(template.format.toUpperCase())}</button></article>`).join("")}</div></section>`;
    $$('[data-capstone-item]', $("#capstoneView")).forEach(input => input.addEventListener("change", () => { state.learner.capstone.milestones[input.dataset.capstoneItem] = input.checked; logActivity(`${input.checked ? "Retained" : "Reopened"} capstone deliverable ${Number(input.dataset.capstoneItem) + 1}`); saveState(); renderCapstone(); renderDashboard(); }));
    $$('[data-template]', $("#capstoneView")).forEach(button => button.addEventListener("click", () => { const template = LEARN.templates.find(item => item.id === button.dataset.template); downloadText(`${template.id}.${template.format}`, templateContent(template), template.format === "csv" ? "text/csv" : "text/markdown"); showToast(`${template.title} downloaded.`); }));
  }
  function exportProgress() { const payload = { exportedAt: new Date().toISOString(), application: "GridReady UK", state }; downloadText(`gridready-progress-${new Date().toISOString().slice(0,10)}.json`, JSON.stringify(payload, null, 2), "application/json"); showToast("Progress export downloaded."); }
  function validateImportedState(candidate) { const imported = candidate?.state || candidate; if (!imported || ![1,2].includes(imported.schema) || !Array.isArray(imported.completed) || !Array.isArray(imported.bookmarks)) throw new Error("This is not a recognised GridReady progress export."); return imported; }
  function previewProgressImport(event) { const file = event.target.files?.[0]; if (!file) return; const preview = $("#importPreview"); const reader = new FileReader(); reader.onload = () => { try { pendingStateImport = validateImportedState(JSON.parse(String(reader.result))); preview.innerHTML = `<div class="import-preview"><strong>Import preview</strong><p>${esc(file.name)}: ${pendingStateImport.completed.length} completed chapters, ${pendingStateImport.bookmarks.length} bookmarks, schema ${esc(pendingStateImport.schema)}. Import replaces the current browser-local GridReady record after confirmation.</p><button class="primary-button" id="confirmImport" type="button">Confirm import</button><button class="text-button" id="cancelImport" type="button">Cancel</button></div>`; $("#confirmImport").addEventListener("click", () => { localStorage.setItem(STORAGE_KEY, JSON.stringify(pendingStateImport)); state = loadState(); pendingStateImport = null; renderAll(); showToast("Progress imported and migrated safely."); }); $("#cancelImport").addEventListener("click", () => { pendingStateImport = null; preview.innerHTML = ""; event.target.value = ""; }); } catch (error) { pendingStateImport = null; preview.textContent = `Import not applied: ${error.message}`; } }; reader.readAsText(file); }
  function resetProgress(scope) { const message = scope === "all" ? "Remove all GridReady data in this browser? Export first if you want a backup." : scope === "lessons" ? "Reset only chapter-reading, bookmarks and lab progress?" : "Reset only diagnostic, route and capstone tracking?"; if (!window.confirm(message)) return; if (scope === "all") state = structuredClone(defaultState); else if (scope === "lessons") { state.completed = []; state.bookmarks = []; state.labProgress = {}; state.lastWeek = 1; } else { state.learner = structuredClone(defaultState.learner); } logActivity(`Reset ${scope} progress`); saveState(); renderAll(); showToast("Requested progress scope reset."); }
  function renderAll() { renderDashboard(); renderStart(); renderPaths(); renderCurriculum(); renderBootcamp(); renderProject(); renderCapstone(); renderTools(); renderGlossary(); renderSources(); renderCPD(); renderTutorHistory(); }

  function renderDashboard() {
    const route = selectedPath(); const routeStatus = pathStatus(route);
    const next = weekFor(routeStatus.next) || DATA.weeks.find(w => !isComplete(w.n)) || DATA.weeks[DATA.weeks.length - 1];
    $("#dashboardView").innerHTML = `
      ...
      <section class="route-dashboard" aria-label="Your next learning action"><div><p class="eyebrow">Your advisory route</p><h2>Path ${esc(route.id)} — ${esc(route.name)}</h2><p>${diagnosticComplete() ? "Diagnostic completed. " : "Complete Start Here diagnostic to refine this recommendation. "}${esc(route.outcome)}</p></div><div><strong>${routeStatus.completed}/${routeStatus.total}</strong><span>mapped chapters read</span><button class="secondary-button" data-go="paths">Review route</button></div></section>
  ...
  function renderSources(filter = "all") {
    ...
    $("#sourcesView").innerHTML = `<div class="page-heading"><div><p class="eyebrow">Authority before memory</p><h1>Source library</h1><p>Official registers and primary documents behind the book. Baseline checked ${esc(DATA.checked)}.</p></div></div>
      ...
      <div class="source-grid">${list.map(s => `<article class="source-card"><div class="source-meta"><span>${esc(s.classification || s.category)}</span><span>Checked ${esc(s.checked)}</span></div><h2>${esc(s.title)}</h2><p><strong>${esc(s.publisher)}</strong><br>${esc(s.edition)}</p><p>${esc(s.note)}</p><details class="source-control"><summary>Source-control details</summary><dl><dt>Status</dt><dd>${esc(s.status || "Verify live status")}</dd><dt>Access</dt><dd>${esc(s.access || "Public")}</dd><dt>Published</dt><dd>${esc(s.published || "See edition")}</dd><dt>Applicability</dt><dd>${esc(s.applicability || "Project-specific review required")}</dd></dl></details><a href="${esc(s.url)}" target="_blank" rel="noopener">Open official source ↗</a></article>`).join("")}</div>`;
  ...
  function navigate(name){ if(name==="dashboard")renderDashboard(); if(name==="start")renderStart(); if(name==="paths")renderPaths(); if(name==="curriculum")renderCurriculum(); if(name==="bootcamp")renderBootcamp(); if(name==="project")renderProject(); if(name==="capstone")renderCapstone(); if(name==="tools")renderTools(); if(name==="glossary")renderGlossary(); if(name==="sources")renderSources(); if(name==="cpd")renderCPD(); showView(name); history.replaceState(null,"",`#${name}`); const titles={dashboard:"Book home",start:"Start Here",paths:"Learning paths",curriculum:"Chapters",bootcamp:"PowerFactory lab manual",project:"Training project",capstone:"Final project",tools:"Engineering tools",glossary:"Glossary",sources:"Source library",cpd:"CPD evidence"};document.title=`${titles[name]||"GridReady UK"} — GridReady UK`;requestAnimationFrame(()=>{const heading=$(`[data-view-panel="${name}"] h1`);if(heading){heading.tabIndex=-1;heading.focus({preventScroll:true});}}); }
  ...
  function init(){
    document.documentElement.dataset.theme=state.theme; buildTutorCorpus(); updateProgressUI(); renderAll();
    ...
    const hash=location.hash.replace("#",""); if(hash.startsWith("chapter-")||hash.startsWith("week-")){const parts=hash.split("/");openWeek(Number(parts[0].replace(/^(chapter-|week-)/,"")),parts[1]||"");} else if(["start","paths","curriculum","bootcamp","project","capstone","tools","glossary","sources","cpd"].includes(hash))navigate(hash); else showView("dashboard");
  }
